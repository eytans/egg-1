...
mod test {
    ...
    prop_compose! {
    fn arb_id(max: u32)(u in 0..=max) -> Id {
        Id(u)
    }
}

    prop_compose! {
    fn arb_node(max_sym: u32, max_children: u32, max_id: u32)(
        sym in arb_symbol(max_sym),
...
//         sym in arb_symbol(max_sym),
//         vec in prop::collection::vec(arb_id(max_id), 0..=(max_children as usize))) -> Expression {
